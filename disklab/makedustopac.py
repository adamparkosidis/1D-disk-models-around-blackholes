from .opacity import bhmie_python as bhmie
import numpy as np
...
def compute_opac_mie(optconst_file, matdens, agraincm, lamcm,
    ...
    if extrapolate:
        ...
        assert np.min(lamcm) >= np.min(
            wavmic * 1e-4), "Error: wavelength range out of range of the optical constants file.\n"
    ...
    if angles[0] == 0.0:
        assert angles[nang -
                      1] == 180, "Error: Angle grid must extend from 0 to 180 degrees."
    else:
        assert angles[0] == 180, "Error: Angle grid must extend from 0 to 180 degrees."
        assert angles[nang -
                      1] == 0, "Error: Angle grid must extend from 0 to 180 degrees."
    #
