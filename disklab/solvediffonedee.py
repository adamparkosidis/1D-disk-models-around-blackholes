import numpy as np
from .tridag import tridag, pentdag


def solvediffonedee(x, y, v, d, g, s, bcl, bcr, dt=None, int=False, upwind=False,
    ...
    if bcl[3] == 0:
        ...
        #
        # Simply set left flux to zero, but implement the equation
        # for the right cell wall
        #
        a[0]  = 0.
        b[0]  =  di[0]  * gi[0]  / (g[0] * dxi[0]  * dxc[0])
        c[0]  = -di[0]  * gi[0]  / (g[1] * dxi[0]  * dxc[0])
        r[0]  = 0.
        if upwind:
            b[0] += (vi[0] >= 0)  * vi[0]  / dxc[0]
            c[0] += (vi[0] < 0)   * vi[0]  / dxc[0]
        else:
            b[0] += 0.5 * vi[0]  / dxc[0]
            c[0] += 0.5 * vi[0]  / dxc[0]
        if dt is not None:
        ...
        print('ERROR in BC in solvediffonedee')
    #
    ...
    if bcr[3] == 0:
        ...
        #
        # Simply set right flux to zero, but implement the equation
        # for the left cell wall
        #
        a[-1]  = -di[-1]  * gi[-1]  / (g[-2] * dxi[-1]  * dxc[-1])
        b[-1]  =  di[-1]  * gi[-1]  / (g[-1] * dxi[-1]  * dxc[-1])
        c[-1]  = 0.
        r[-1]  = 0.
        if upwind:
            a[-1] -= (vi[-1] >= 0) * vi[-1] / dxc[-1]
            b[-1] -= (vi[-1] < 0)  * vi[-1] / dxc[-1]
        else:
            a[-1] -= 0.5 * vi[-1] / dxc[-1]
            b[-1] -= 0.5 * vi[-1] / dxc[-1]
        if dt is not None:
...
def getfluxonedee(x, y, v, d, g, int=False, upwind=False):
    """
