...
ddum    = DiskRadialModel()
t0      = ddum.compute_shuulrich_infall(mcloud, cscloud, omcloud, 0.,get_tdisk=True)
mstar0  = ddum.infall_mdot*t0

time = tstart * ((tend-t0) / tstart)**(np.linspace(0., 1., ntime))
time = np.hstack((0.,time)) + t0
...
for itime in range(1, ntime + 1):
    ...
    d.gravinstab_flattening(flushgridedges=True)
...
itimes = [1,250,500,750,999]

plt.figure()
for it in range(len(itimes)):
    plt.loglog(d.r/au,dlist[itimes[it]].sigma,label='t = {0:9.2e} year'.format(time[itimes[it]]/year))
plt.xlim(0.1,1e3)
plt.ylim(1e-5,1e6)
plt.xlabel('r [au]')
...
plt.figure()
for it in range(len(itimes)):
    plt.loglog(d.r/au,dlist[itimes[it]].qtoomre,label='t = {0:9.2e} year'.format(time[itimes[it]]/year))
plt.xlim(0.1,1e3)
plt.ylim(1e-1,1e2)
plt.xlabel('r [au]')
